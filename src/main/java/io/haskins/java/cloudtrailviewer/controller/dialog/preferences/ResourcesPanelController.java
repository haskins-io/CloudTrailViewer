...
/**
 * Controller class that handles the Resources Panel.
 *
...
public class ResourcesPanelController extends PreferencesPanel {
    ...
    public void init(DatabaseService databaseService) {
        ...
        String query = "SELECT api_call FROM aws_resources ORDER BY api_call ASC";
        populateListBySQL(query);
    }

    void removeItem(String itemToRemove) {

        String query = "DELETE FROM aws_resources WHERE api_call = '" + itemToRemove + "'";
