 ...
 * items in an underlying dataset. The index of the first "visible" item can be
 * modified, which provides a means of "sliding" through the items in the
 * underlying series dataset.
 */
public class SlidingXYDataset extends AbstractXYDataset
    ...
    /**
     * Storage for the series offset from the items at 0. This list must be kept
     * in sync with the seriesList.
     */
    private int[] seriesOffset;

    /**
     * Creates a new <code>SlidingXYDataset</code> class that applies a
    ...
    /**
     * Gets the off set of the series from the left most item in the dataset.
     * This assumes the domain(X) values to be in equal intervals.
     *
    ...
    /**
     * Gets the interval. It is the difference between to consecutive x values.
     *
     * This assumes the domain(X) values to be in equal intervals.
     *
     *
    ...
    /**
     * Sets the index of the first item that should be used from the underlying
     * dataset, and sends a {@link DatasetChangeEvent} to all registered
     * listeners.
     *
    ...
    /**
     * Returns the x-value for an item within a series using the offsets
     * relative positions in the underlying dataset.
     *
     * @param series the series index (in the range <code>0</code> to
     * <code>getSeriesCount() - 1</code>).
    ...
    /**
     * Gets the corresponding item index in the underlying dataset.
     *
    ...
    /**
     * Returns the y-value for an item within a series using the offsets
     * relative positions in the underlying dataset.
     *
     * @param series the series index (in the range <code>0</code> to
     * <code>getSeriesCount() - 1</code>).
     ...
     * the same values, window and firstItemIndex.</li>
     * </ul>
     *
